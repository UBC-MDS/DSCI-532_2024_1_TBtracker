...
tb_data_preprocess["incidence_total"] = tb_data_preprocess["e_inc_num"] + \
    tb_data_preprocess["e_inc_num_hi"]
tb_data_preprocess["incidence_rate"] = tb_data_preprocess["incidence_total"] / \
    tb_data_preprocess["e_pop_num"]
tb_data_preprocess["mortality_total"] = tb_data_preprocess["e_mort_exc_tbhiv_num"] + \
    tb_data_preprocess["e_mort_tbhiv_num"]
tb_data_preprocess["mortality_rate"] = tb_data_preprocess["mortality_total"] / \
    tb_data_preprocess["e_pop_num"]
tb_data_preprocess["cfr_total"] = tb_data_preprocess["cfr"]
tb_data_preprocess["tbhiv_coinfection_total"] = tb_data_preprocess["e_inc_tbhiv_num"] + \
    tb_data_preprocess["e_inc_tbhiv_num_hi"]
tb_data_preprocess["tbhiv_coinfection_rate"] = tb_data_preprocess["tbhiv_coinfection_total"] / \
    tb_data_preprocess["e_pop_num"]

# Creating all combinations of countries and years
countries = tb_data_preprocess["country"].unique()
years = range(tb_data_preprocess["year"].min(),
              tb_data_preprocess["year"].max() + 1)
all_combinations = pd.MultiIndex.from_product(
    [countries, years], names=["country", "year"]).to_frame(index=False)
tb_data_preprocess = pd.merge(all_combinations, tb_data_preprocess, on=[
                              "country", "year"], how="left")

# Handling missing and filling values
tb_data_preprocess["iso_numeric"] = tb_data_preprocess.groupby(
    "country")["iso_numeric"].transform(lambda x: x.fillna(x.max()))
...
# Save to Parquet
try:
    tb_data_preprocess.to_parquet(
        "data/processed/tb_data.parquet", index=False)
...
try:
    ...
    rf_data_preprocess = rf_data[(rf_data["sex"] != "a") & (
        rf_data["age_group"] != "all")].copy()
    rf_data_preprocess = rf_data_preprocess[~rf_data_preprocess["age_group"].isin(
        ["0-14", "15plus", "18plus"])]
    ...
    rf_data_preprocess.to_parquet(
        "data/processed/rf_data.parquet", index=False)
    ...
    filtered_data_2022 = filtered_data_2022.loc[
        (filtered_data_2022["age_group"].isin(["all", "18plus", "15plus"])) & (
            filtered_data_2022["sex"] == "a") & (filtered_data_2022["risk_factor"] != "all")
    ]
    pivot_data = filtered_data_2022.pivot_table(
        index=["country"], columns="risk_factor", values="best", aggfunc="sum").reset_index()
    pivot_data.fillna(0, inplace=True)
    pivot_data.to_parquet("data/processed/rf_type_data.parquet", index=False)
