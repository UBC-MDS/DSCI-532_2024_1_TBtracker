...
def update_global_stats(selected_year, selected_type, selected_value):
    ...
    if selected_year != 2000:
        ...
        if global_stat_previous:
            diff_previous = round(
                ((global_stat - global_stat_previous) / global_stat_previous) * 100, 1
            )
    ...
    if selected_year != 2022:
        ...
        if global_stat_next:
            diff_next = round(
                ((global_stat - global_stat_next) / global_stat_next) * 100, 1
            )
            diff_next_color = "blue" if diff_next > 0 else "red"

    diff_previous_text = (
        f"{diff_previous:+.1f}%" if diff_previous is not None else "data not available"
    )
    diff_next_text = (
